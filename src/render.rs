...
void main() {
        ...
        for robot in robots {
            ...
            if let Ok(target_distance) = robot.vm_state.registers.get(Register::TargetDistance) {
                if target_distance > 0.0 {
                    if let Ok(target_direction_deg) =
                        robot.vm_state.registers.get(Register::TargetDirection)
                    {
    ...
    fn draw_robot(
        ...
        let target_drive_deg =
            (robot.drive.direction + robot.drive.pending_rotation).rem_euclid(360.0) as f32;
        let target_turret_deg =
            (robot.turret.direction + robot.turret.pending_rotation).rem_euclid(360.0) as f32;
        ...
        let turret_end =
            center_pos + Vec2::new(turret_rad.cos(), turret_rad.sin()) * radius * 2.0 * 0.8;
        draw_line(
            center_pos.x,
            center_pos.y,
            turret_end.x,
    ...
    fn draw_ui_panel(
        ...
        for robot in robots {
            ...
            for i in 0..filled_health_segments {
                let segment_x =
                    panel_x + card_inner_padding_x + (segment_width + segment_gap) * i as f32;
            ...
            let power_segment_width =
                (card_bar_width - total_power_gap_width) / num_power_segments as f32;
            ...
            for i in 0..filled_power_segments {
                let segment_x = panel_x
                    + card_inner_padding_x
                    + (power_segment_width + power_segment_gap) * i as f32;
                // Clamp width for the last potentially partial segment
                let current_segment_width = if i == filled_power_segments - 1 {
                    (card_bar_width * power_ratio)
                        - ((power_segment_width + power_segment_gap) * i as f32)
                } else {
                    power_segment_width
                }
    ...
    fn draw_announcement(&self, msg: &str) {
        ...
        let text_y =
            y + (rect_height - font_size_announcement) / 2.0 + font_size_announcement * 0.7; // Adjust Y for better centering
